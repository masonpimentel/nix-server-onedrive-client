...
def fs_get_local_filename():
    ...
    return os.path.basename(paths["upload_pairs"][0]["local_tar"])
...
def fs_get_upload_size():
    paths = config_get_paths()
    try:
        size = os.stat(paths["upload_pairs"][0]["local_tar"]).st_size
    except FileNotFoundError:
        print_message("File '" + paths["upload_pairs"][0]["local_tar"] + "' could not be found!", "UPLOAD", "error")
        return None
    return size
